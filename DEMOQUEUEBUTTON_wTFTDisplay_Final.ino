...
void loop() {
  if (currentLED == -1) {
    if (digitalRead(BTN_REGULAR) == LOW) {
      ...
      activateLED(LED_GOOD, 2500, "Feedback:\n\nGood.\n\nThank you!", "34", false);
      delay(200); // Debounce
    } else if (digitalRead(BTN_POOR) == LOW) {
      activateLED(LED_POOR, 2500, "Feedback:\n\nPoor.\n\nWe're sorry to \n\nhear that!", "35", false);
      delay(200); // Debounce
    }
  }
...
void activateLED(int ledPin, int duration, const char* message, const char* label, bool isTicket) {
  ...
  Serial.println(label); // Send the correct label for index.js
}
